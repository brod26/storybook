...
export class PreviewWeb<TFramework extends AnyFramework> {
  ...
  getProjectAnnotationsOrRenderError(
    ...
    if (sync) {
      try {
        return this.setProjectAnnotations(
          (getProjectAnnotations as () => WebProjectAnnotations<TFramework>)()
        );
      } catch (err) {
        logger.warn(err);
    ...
    return Promise.resolve()
      ...
      .then((projectAnnotations) => this.setProjectAnnotations(projectAnnotations))
      .catch((err) => {
        logger.warn(err);
        // This is an error extracting the projectAnnotations (i.e. evaluating the previewEntries) and
        // needs to be show to the user as a simple error
  ...
  setProjectAnnotations(projectAnnotations: WebProjectAnnotations<TFramework>) {
